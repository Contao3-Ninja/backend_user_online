 ...
 * @license    LGPL 
 * @version    $Id: tl_member.php 97 2010-06-16 19:15:46Z bibo $
 */
 ...
 * Listing
 */
//$GLOBALS['TL_DCA']['tl_member']['list']['label']['label_callback'][0] = 'tl_member_onlineicon';
$GLOBALS['TL_DCA']['tl_member']['list']['label']['label_callback'] = array('tl_member_onlineicon','addIcon');

/**
 * Class tl_member_onlineicon (Frontend)
 *
 * @copyright  Glen Langer 2009..2011 
 * @author     Glen Langer 
 * @package    BackendUserOnline
...
class tl_member_onlineicon extends Backend
{
	...
	public function addIcon($row, $label)
	{
		...
		else
		{
		    ...
		    $lastLogin = '<span style="float: right; color: #B3B3B3;">['.$GLOBALS['TL_LANG']['MSC']['tl_member_onlineicon']['lastlogin'].': '.$GLOBALS['TL_LANG']['MSC']['tl_member_onlineicon']['lastlogin_never'].']</span>';
		}
		...
		if ($objUsers->numRows < 1) {
			...
			if(version_compare(VERSION . '.' . BUILD, '2.7.6', '>'))
			{
				if ($row['currentLogin'] >0 ) {
					$lastLogin = '<span style="float: right; color: #B3B3B3;">['.$GLOBALS['TL_LANG']['MSC']['tl_member_onlineicon']['lastlogin'].': '.date($GLOBALS['TL_CONFIG']['datimFormat'], $row['currentLogin']).']</span>';
				}
			}
		} else {
			//online
